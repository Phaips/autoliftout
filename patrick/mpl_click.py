...
def relabel_img(img, title="Relabel"):
...
class DataEngine:
    def __init__(self, images):
        ...
        img_plot = plt.imshow(images[0], cmap="gray")

        fig.suptitle("Data Engine")
...
def select_point(image):
    ...
    plt.show()
    return np.flip(coords[-2:], axis=0)
...
if __name__ == "__main__":
    ...
    # filenames = glob.glob("data/aug/*/img.png")
    filenames = glob.glob("patrick/*.png")
    shuffle(filenames)
    ...
    for img in images:
        ...
        for label in ["lamella", "needle"]:
            # label_json = relabel_img(img, title=f"Relabel centre of {label}")

            # print(label, ": ", label_json)
            # Returns dictionary:
            # {"label" : "label", "points": [[x1, y1], [x2, y2]]
            # todo; change needle selection to point
            ...
            # just simplify this to start with
            # if wrong detection:
                # click centre of lamella
                # click needle tip
                # save image
                # continue
