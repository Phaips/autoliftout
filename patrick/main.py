...
def take_electron_and_ion_reference_images(microscope, hor_field_width=50e-6,  image_settings=None, image_settings_electron=None, image_settings_ion=None):
    ...
    if image_settings_electron not None:
        image_settings = image_settings_electron 
    eb_reference = new_electron_image(microscope, image_settings)
    #
    microscope.imaging.set_active_view(2)
    autocontrast(microscope, beam_type=BeamType.ION)
    if image_settings_ion not None:
        image_settings = image_settings_ion 
...
def sputter_platinum_over_whole_grid(microscope):
    ...
    sputter_platinum(microscope, sputter_time=10, horizontal_field_width=30e-6, line_pattern_length=7e-6)
...
def land_needle_on_milled_lamella(microscope, needle_reference_images_nobg, move_in_x=True, move_in_y=True, xcorrection=1e-6, ycorrection=1.5e-6):
    from autoscript_sdb_microscope_client import SdbMicroscopeClient
    from autoscript_sdb_microscope_client.structures import AdornedImage, GrabFrameSettings
    from autoscript_sdb_microscope_client.structures import ManipulatorPosition
    ...
    stage  = microscope.specimen.stage
    needle = microscope.specimen.manipulator
    needle_ref_eb_lowres_nobg  = needle_reference_images_nobg[0]
    needle_ref_eb_highres_nobg = needle_reference_images_nobg[1]
    needle_ref_ib_lowres_nobg  = needle_reference_images_nobg[2]
    needle_ref_ib_highres_nobg = needle_reference_images_nobg[3]
    storage.SaveImage(needle_ref_eb_lowres_nobg,  id='A_needle_land_needle_ref_eb_lowres' )
    storage.SaveImage(needle_ref_eb_highres_nobg, id='A_needle_land_needle_ref_eb_highres' )
    storage.SaveImage(needle_ref_ib_lowres_nobg,  id='A_needle_land_needle_ref_ib_lowres' )
    storage.SaveImage(needle_ref_ib_highres_nobg, id='A_needle_land_needle_ref_ib_highres' )
    ######
    #################### LOW & HIGH RES IMAGES ########################
    ...
    needle_eb_lowres_with_lamella, needle_ib_lowres_with_lamella = take_electron_and_ion_reference_images(microscope, hor_field_width=field_width_lowres, image_settings=image_settings)
    ###
    ...
    needle_eb_highres_with_lamella, needle_ib_highres_with_lamella = take_electron_and_ion_reference_images(microscope, hor_field_width=field_width_highres, image_settings=image_settings)
    ###
    ...
    x_shift, y_shift = find_needletip_shift_in_image_ELECTRON(needle_eb_lowres_with_lamella, needle_ref_eb_lowres_nobg,
                                                               show=False, median_smoothing=2)
    ...
    xcorrection = 1e-6 # empirically found
    ycorrection = 2e-6 # empirically found
    ...
    print('Needle approach from e-beam low res:')
    print('x_move = ', x_move, ';\ny_move = ', y_move)
    needle.relative_move(x_move)
    ...
    pixelsize_x_lowres = needle_ref_eb_lowres_nobg.metadata.binary_result.pixel_size.x
    field_width_lowres = pixelsize_x_lowres * needle_ref_eb_lowres_nobg.width
    needle_eb_lowres_with_lamella_shifted, needle_ib_lowres_with_lamella_shifted = take_electron_and_ion_reference_images(microscope, hor_field_width=field_width_lowres, image_settings=image_settings)
    ########
    pixelsize_x_highres = needle_ref_eb_highres_nobg.metadata.binary_result.pixel_size.x
    field_width_highres = pixelsize_x_highres * needle_ref_eb_highres_nobg.width
    needle_eb_highres_with_lamella_shifted, needle_ib_highres_with_lamella_shifted = take_electron_and_ion_reference_images(microscope, hor_field_width=field_width_lowres, image_settings=image_settings)
    ###
    ...
    pixelsize_x_highres = needle_ref_eb_highres_nobg.metadata.binary_result.pixel_size.x
    field_width_highres = pixelsize_x_highres * needle_ref_eb_highres_nobg.width
    needle_eb_highres_with_lamella_shifted, needle_ib_highres_with_lamella_shifted = take_electron_and_ion_reference_images(microscope, hor_field_width=field_width_highres, image_settings=image_settings)
    # save
    storage.SaveImage(needle_eb_lowres_with_lamella_shifted,  id='D_needle_land_sample_eb_lowres_shifted' )
    storage.SaveImage(needle_eb_highres_with_lamella_shifted, id='D_needle_land_sample_eb_highres_shifted')
    storage.SaveImage(needle_ib_lowres_with_lamella_shifted,  id='D_needle_land_sample_ib_lowres_shifted' )
    storage.SaveImage(needle_ib_highres_with_lamella_shifted, id='D_needle_land_sample_ib_highres_shifted')
    ##########
    ##########
    x_shift, y_shift = find_needletip_shift_in_image_ION(needle_ib_lowres_with_lamella_shifted, needle_ref_ib_lowres_nobg,
                                                          show=False, median_smoothing=2)
    ...
    print('Needle move in Z by half...')
    needle.relative_move(zy_move_half)
    print('Needle move in Z by half... LANDED')
    needle.relative_move(zy_move_half)
    ...
    pixelsize_x_lowres = needle_ref_eb_lowres_nobg.metadata.binary_result.pixel_size.x
    field_width_lowres = pixelsize_x_lowres * needle_ref_eb_lowres_nobg.width
    needle_eb_lowres_with_lamella_landed, needle_ib_lowres_with_lamella_landed = take_electron_and_ion_reference_images(microscope, hor_field_width=field_width_lowres, image_settings=image_settings)
    ###
    pixelsize_x_highres = needle_ref_eb_highres_nobg.metadata.binary_result.pixel_size.x
    field_width_highres = pixelsize_x_highres * needle_ref_eb_highres_nobg.width
    needle_eb_highres_with_lamella_landed, needle_ib_highres_with_lamella_landed = take_electron_and_ion_reference_images(microscope, hor_field_width=field_width_highres, image_settings=image_settings)
...
########################################LIFT-OUT#########################################################
def liftout_lamella(microscope, settings, needle_reference_imgs_nobg):
    from autoscript_sdb_microscope_client.structures import AdornedImage, GrabFrameSettings
    from autoscript_sdb_microscope_client.structures import ManipulatorPosition
    needle = microscope.specimen.manipulator
    stage = microscope.specimen.stage
    ...
    park_position = move_needle_to_liftout_position(microscope)
    land_needle_on_milled_lamella(microscope, needle_reference_imgs_nobg, move_in_x=True, move_in_y=True)
    ...
    retract_needle(microscope, park_position)
    return needle_reference_images_highres_with_lamella
...
def mill_lamella(microscope, settings, confirm=True):
    ...
    #eb_lowres_reference,  ib_lowres_reference  = take_electron_and_ion_reference_images(microscope, hor_field_width=400e-6, image_settings=image_settings) # TODO: yaml use input
    ...
    resolution          = storage.settings["reference_images"]["milled_lamella_alignment_img_resolution"]
    dwell_time_electron = storage.settings["reference_images"]["milled_lamella_alignment_img_dwell_time_electron"]
    dwell_time_ion      = storage.settings["reference_images"]["milled_lamella_alignment_img_dwell_time_ion"]
    hfw_highres         = storage.settings["reference_images"]["milled_lamella_alignment_img_hfw_highres"]
    hfw_lowres          = storage.settings["reference_images"]["milled_lamella_alignment_img_hfw_lowres"]
    image_settings_electron = GrabFrameSettings(resolution=resolution, dwell_time=dwell_time_electron)
    image_settings_ion      = GrabFrameSettings(resolution=resolution, dwell_time=dwell_time_ion)
    eb_lowres_reference,  ib_lowres_reference  = take_electron_and_ion_reference_images(microscope, hor_field_width=hfw_lowres, image_settings_electron=image_settings_electron, image_settings_ion=image_settings_ion) # TODO: yaml use input
    eb_highres_reference, ib_highres_reference = take_electron_and_ion_reference_images(microscope, hor_field_width=hfw_highres,image_settings_electron=image_settings_electron, image_settings_ion=image_settings_ion) # TODO: yaml use input
    ...
    #eb_highres_reference, ib_highres_reference = take_electron_and_ion_reference_images(microscope, hor_field_width=50e-6,  image_settings=image_settings) # TODO: yaml use input
    resolution          = storage.settings["reference_images"]["aligned_lamella_ref_img_resolution"]
    dwell_time_electron = storage.settings["reference_images"]["aligned_lamella_ref_img_dwell_time_electron"]
    dwell_time_ion      = storage.settings["reference_images"]["aligned_lamella_ref_img_dwell_time_ion"]
    hfw_highres         = storage.settings["reference_images"]["aligned_lamella_ref_img_hfw_highres"]
    hfw_lowres          = storage.settings["reference_images"]["aligned_lamella_ref_img_hfw_lowres"]
    image_settings_electron = GrabFrameSettings(resolution=resolution, dwell_time=dwell_time_electron)
    image_settings_ion      = GrabFrameSettings(resolution=resolution, dwell_time=dwell_time_ion)
    eb_lowres_reference,  ib_lowres_reference  = take_electron_and_ion_reference_images(microscope, hor_field_width=hfw_lowres, image_settings_electron=image_settings_electron, image_settings_ion=image_settings_ion) # TODO: yaml use input
    eb_highres_reference, ib_highres_reference = take_electron_and_ion_reference_images(microscope, hor_field_width=hfw_highres,image_settings_electron=image_settings_electron, image_settings_ion=image_settings_ion) # TODO: yaml use input
    ...
    ############################################################################################
    # Take new images, use them as reference to align for the next 3 deg tilt
    ...
    image_settings_electron = GrabFrameSettings(resolution=resolution, dwell_time=dwell_time_electron)
    image_settings_ion      = GrabFrameSettings(resolution=resolution, dwell_time=dwell_time_ion)
    eb_lowres_ref,  ib_lowres_ref  = take_electron_and_ion_reference_images(microscope, hor_field_width=hfw_lowres, image_settings_electron=image_settings_electron, image_settings_ion=image_settings_ion) # TODO: yaml use input
    eb_highres_ref, ib_highres_ref = take_electron_and_ion_reference_images(microscope, hor_field_width=hfw_highres,image_settings_electron=image_settings_electron, image_settings_ion=image_settings_ion) # TODO: yaml use input    
    ...
    ### Need to tilt +6 deg, tilt first +3 deg only, again +23deg, Now +6 deg
    previous_stage_tilt = stage.current_position.t
    ...
    eb_lowres_ref_jcut,  ib_lowres_ref_jcut  = take_electron_and_ion_reference_images(microscope, hor_field_width=hfw_lowres, image_settings_electron=image_settings_electron, image_settings_ion=image_settings_ion) # TODO: yaml use input
    eb_highres_ref_jcut, ib_highres_ref_jcut = take_electron_and_ion_reference_images(microscope, hor_field_width=hfw_highres,image_settings_electron=image_settings_electron, image_settings_ion=image_settings_ion) # TODO: yaml use input    
...
def land_lamella(microscope, landing_coord, original_landing_images, needle_reference_images_highres_with_lamella):
    from autoscript_sdb_microscope_client import SdbMicroscopeClient
    from autoscript_sdb_microscope_client.structures import AdornedImage, GrabFrameSettings
    from autoscript_sdb_microscope_client.structures import ManipulatorPosition
    stage = microscope.specimen.stage
    ...
    #manual_needle_movement_in_xy(microscope, original_landing_images, [], move_in_x=False)
...
def single_liftout(microscope, settings, landing_coord, lamella_coord, original_landing_images, original_lamella_area_images):
    ...
    storage.NEEDLE_REF_IMGS = needle_reference_imgs

    ### move to the previously stored position and correct the position using the reference images:
    microscope.specimen.stage.absolute_move(lamella_coord)       #realign_sample_stage(microscope, image, template, beam_type=BeamType.ION, correct_z_height=False)
    ...
    # lift-out
    needle_reference_images_highres_with_lamella = liftout_lamella(microscope, settings, needle_reference_imgs)

    # land
    land_lamella(microscope, original_landing_images, needle_reference_images_highres_with_lamella)
