...
class GUIMainWindow(gui_main.Ui_MainWindow, QtWidgets.QMainWindow):
    ...
    def update_image_settings_v3(
        ...
        image_settings = acquire.ImageSettings(
            ...
            save_path=save_path if save_path is None else save_path,
            label=datetime.datetime.fromtimestamp(time.time()).strftime("%Y%m%d.%H%M%S")
            if label is None
            else label,
        )
        ...
        # change the save path to the current sample if available
        # if current_sample_position:
        #     image_settings.save_path = os.path.join(
        #         save_path, str(current_sample_position.sample_id)
        #     )
