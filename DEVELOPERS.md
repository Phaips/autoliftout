...
### Fork then clone the `liftout` GitHub repository
If you don't already have a GitHub account, you can create one here:
<https://github.com/join>

**First, fork the `liftout` repository** at
<https://github.com/DeMarcoLab/liftout>
...
```
git clone https://github.com/$YOUR_GITHUB_USERNAME/liftout.git
cd autolamella
git remote add upstream https://github.com/DeMarcoLab/liftout.git
```
...
```
You can confirm you are on the correct `develop` branch of the liftout repository by using the command `git branch`.
...
```
conda create -n fibsem-dev python=3.6 pip
conda activate fibsem-dev
...
2. Find the conda environment location you just made called `fibsem-dev`.
`...conda/envs/fibsem-dev/Lib/site-packages/`

***Troubleshooting:** If you're having trouble finding the conda environment location for `fibsem-dev`*
*you can open the *Anaconda terminal* on your machine and type `where python` (Windows) or `which python` (Unix).*
*The result will be something like `C:\Users\yourusername\.conda\envs\fibsem-dev\python.exe`*
*Navigate to the environment location (in the example here, that's `C:\Users\yourusername\.conda\envs\fibsem-dev\` *
...
### Install liftout as an editable installation

Last, pip install `liftout` as an editable installation:
```
conda activate fibsem-dev
pip install -e .
```
